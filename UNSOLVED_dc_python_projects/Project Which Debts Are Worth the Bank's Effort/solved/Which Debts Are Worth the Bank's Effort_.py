...
'''
the Kruskal-Wallis test is used for comparing two or more independent samples of equal or
different sample sizes. It extends the Mann–Whitney U test, 
which is used for comparing only two groups. The parametric equivalent 
...
based on test anxiety levels (i.e., your dependent variable would be
"exam performance" and your independent variable would be "test anxiety level",
which has three independent groups: students with "low", "medium" and "high"
test anxiety levels).
...
'''
The chi-square test is often used to see if two categorical variables are 
independent or dependent. If they are independent, the p-value is not likely 
to be statistically significant while if they are dependent,
the p-value is more likely to be significant (for example, less than 0.01).
